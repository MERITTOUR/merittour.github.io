...
**안내문을 열면 아직 안 올린 자리가 점선으로 보입니다.** 점선에 폴더 이름이 적혀 있고,
블록 아래에 앞에 붙일 경로가 적혀 있습니다 — 문서를 다시 뒤질 필요가 없습니다.
...
- **구마모토**는 숙소 카드에 `<li data-lg="폴더이름">` 을 붙여 씁니다. 이름은 카드에 적힌
  것을 그대로 읽으므로 따로 적지 않습니다.
- **그 밖 지역**은 `<div class="region" id="r-…" data-stay="폴더:이름|폴더:이름">` 에 적습니다.
- 어느 쪽이든 **HTML 한 곳만** 고치면 점선 자리가 따라 생깁니다.
