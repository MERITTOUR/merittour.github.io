...
**`01.webp` ~ `04.webp`** 이름으로 넣으면 코드를 고치지 않아도 그 자리에 바로 나타납니다.
GitHub 웹에서 **Add file → Upload files** 로 드래그하면 됩니다.

```
2027/photos/kumamoto/prestige/01.webp    ← 야마나미 프레스티지
...
| 지역 | 폴더 | 숙소 | 폴더 |
|---|---|---|---|
| 구마모토 · 아소 야마나미 | `kumamoto` | 야마나미 프레스티지 | `prestige` |
| | | 야마나미 히노키빌라 – 캐빈 | `hinoki-cabin` |
| | | 야마나미 히노키빌라 – 패밀리 | `hinoki-family` |
| | | 야마나미 프라이빗 레지던스 | `residence` |
| | | 야마나미 호텔 | `hotel` |
| | | 간지호텔 | `guernsey` |
...
```html
<li data-lg="prestige">
  <div class="lg-name">야마나미 프레스티지 <span class="lg-jp">YAMANAMI PRESTIGE HOUSE</span></div>
...
| 장수 | 숙소마다 **최대 4장** | `01`~`04`. 1~2장만 올려도 됩니다 |
...
- **숙소 사진은 그 숙소 카드 안**, 설명 바로 아래에 한 줄로 깔립니다.
  세로로 쌓지 않고 **옆으로 밀어서** 봅니다 — 폰은 손가락으로, 데스크톱은 마우스로 끌면 됩니다.
- **누르면 화면 가득 크게** 뜹니다. 좌우 화살표(또는 `←` `→` 키)로 넘기고, 배경을 누르거나 `Esc` 로 닫습니다.
  크게 보기는 **그 숙소 사진 안에서만** 넘어갑니다 — 옆 숙소 사진이 섞이지 않습니다.
- **골프장 사진은 지역마다 하나**라 「골프장 정보 보기」로 접어 둡니다.
  펼쳐 둔 채로 한참 내려가면 **스스로 접힙니다** — 그 아래 요금·일정을 찾기까지 한참 굴리지 않도록.
- 사진은 **그 숙소가 화면에 가까워질 때** 받습니다. 문서를 여는 순간에는 한 장도 받지 않습니다.
